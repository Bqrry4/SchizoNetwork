...
void scan() {
    ...
    while (true) {
        ...
        FILE *nmap_pipe = popen("nmap -n 192.168.0.0/24 -p 35544 -oG - | awk '/Up$/{print $2}'", "r");
        printf("Scan started...\n");

        while (fgets(nmap_temp_buffer, 4096, nmap_pipe) != NULL) {
            strcat(final_buffer, nmap_temp_buffer);
        }
...
void print_progress(int blocks_received, int blocks_total) {
    ...
    int progress = blocks_received * 50 / blocks_total;
    memset(progress_bar, '#', progress);

    printf("\rProgress: [%s]", progress_bar);
    fflush(stdout);
}
